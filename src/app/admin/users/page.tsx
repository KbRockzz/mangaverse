...
export default function AdminUsersPage() {
  ...
  const updateRoleMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.success) { addToast("success", "Role updated!"); qc.invalidateQueries({ queryKey: ["admin-users"] }); }
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { addToast("success", "User deleted"); qc.invalidateQueries({ queryKey: ["admin-users"] }); setDeleteModal(null); },
  });
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 220 }}>
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            placeholder="Search by name or email..." className="form-input" style={{ paddingLeft: "2.5rem" }} />
        </div>
        <select value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value); setPage(1); }} className="form-input" style={{ maxWidth: 160 }}>
                  ...
                  <tr key={user.id}>
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                        <div style={{
                          width: 36, height: 36, borderRadius: "50%", background: "var(--accent-color)",
                    ...
                    <td>
                      <button className="btn-danger btn-sm" onClick={() => setDeleteModal(user)}><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
